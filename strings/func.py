...
async def mplay_stream(message,MusicData):
    ...
    mystic = await message.reply_text(f"◍ جاري البحث....")
    await mystic.edit(
        f"{title[:50]}"
    )
    ...
    thumb = await gen_thumb(
                        thumbnail, title, message.from_user.id, "SHaDoW PlAyInG", views, duration_min, channel
                    )
...
async def custom_start_stream(
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            photo=thumb,
            caption=(
                f"💡 **تمت إضافة المقطع إلى قائمة الانتظار »** `{position}`\n\n🏷 **الاسم:** {title}\n**⏱ المده:** `{duration_min}`\n🎧 **طلب بواسطة:** {message.from_user.mention}"
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        await mystic.delete()        
        ...
        if not await join_stream(message.chat.id, file):
            return await mystic.edit("◍ قم بفتح الكول اولا فى الجروب\n√")
...
async def custom_video_stream(
    ...
    if await is_active_chat(message.chat.id):
        ...
        if not await join_video_stream(
            ...
            return await message.reply_text(
                f"◍ قم بفتح الكول اولا فى الجروب\n√"
            )
        get_queue[message.chat.id] = []
        got_queue = get_queue.get(message.chat.id)
        title = title
